...
class TaoBao(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        self.convertResponse(kindlist, content, pattern)
        ...
        for kind in kindlist:
            ...
            item['name'] = itemName.group(0)[1:-1]
            item['url'] = linkContent[:-1]
            yield item

    def convertResponse(self,kindlist, content, pattern):
        newConetent = self.convert(kindlist, content, pattern, 0)
        if newConetent is not None:
            self.convertResponse(kindlist, newConetent, pattern)
